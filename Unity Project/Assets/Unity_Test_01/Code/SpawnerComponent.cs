...
namespace Tangelo.Test.Components
{
    ...
    {
        //To start the game will populate the pool
        void PopulatePool(int number_of_objects, Slider ui_slider);
        //For every spawn
        void Spawn(Vector3 position, int number_object_total_spawn);
        //Recycle gameobjects
        void Recycle(int number_of_objects);
    }
    ...
    public class SpawnerComponent : MonoBehaviour, ISpawnerComponent
    {
        ...
        public void PopulatePool(int number_of_objects, Slider ui_slider)
        {
            ...
            {
                GameObject temporal_gameobject = GameObject.CreatePrimitive(PrimitiveType.Cube);
        ...
        public void Spawn(Vector3 position, int number_object_total_spawn)
        {
            ...
            for (int i=0; i< spawn_pool.Length; i++)
            {
                if (spawn_pool[i].activeSelf == false)
                {
                    ...
                    temporal_gameobject.transform.localScale = Vector3.one;
            ...
            if (need_more_memory == true)
            {
                ...
                {
                    GameObject temporal_gameobject_to_pool = GameObject.CreatePrimitive(PrimitiveType.Cube);
